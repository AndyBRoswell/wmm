    ...
    * const TargetDoc) :
    ...
    void JSONHiliter::Highlight(const QString& Text) {
        ...
        if (StrBeginPos.empty()) {
            ...
            for (const auto& HighlightRule: HighlightRules) {
//        qDebug() << "Current Pattern: " << HighlightRule.Pattern;
                {
                    QRegularExpressionMatchIterator MatchIterator = HighlightRule.Pattern.globalMatch(TextRef.sliced(0, StrBeginPos[0]));
//                qDebug() << "Match range: " << TextRef.sliced(0, StrBeginPos[0]));
                    while (MatchIterator.hasNext()) {
                        QRegularExpressionMatch Match = MatchIterator.next();
                        setFormat(Match.capturedStart(), Match.capturedLength(), HighlightRule.Format);
//                qDebug() << "Matched: " << TextRef.sliced(Match.capturedStart(), Match.capturedLength());
                    }
                }
...
//            qDebug() << "Match range: " << TextRef.sliced(StrEndPos[i], StrBeginPos[i + 1] - StrEndPos[i]);
                    while (MatchIterator.hasNext()) {
                        QRegularExpressionMatch Match = MatchIterator.next();
                        setFormat(StrEndPos[i] + Match.capturedStart(), Match.capturedLength(), HighlightRule.Format);
//                qDebug() << "Matched: " << TextRef.sliced(StrEndPos[i] + Match.capturedStart(), Match.capturedLength());
                    }
                }
                {
...
//                qDebug() << "Match range: " << TextRef.sliced(StrEndPos[StrEndPos.size() - 1]);
                    while (MatchIterator.hasNext()) {
                        QRegularExpressionMatch Match = MatchIterator.next();
                        setFormat(StrEndPos[StrEndPos.size() - 1] + Match.capturedStart(), Match.capturedLength(), HighlightRule.Format);
//                qDebug() << "Matched: " << TextRef.sliced(StrEndPos.size() - 1 + Match.capturedStart(), Match.capturedLength());
                    }
                }
            }
        }
